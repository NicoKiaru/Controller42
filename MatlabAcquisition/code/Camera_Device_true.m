classdef Camera_Device < Device
    ...
    methods
        ...
        function buildGui(self)
                ...
                % Label for images skipping
                uicontrol('style','text',...
                          'position',[600 20 100 20],...
                          'string','Images skipped');
                
                % Exposure time control (edit text)
                 self.edit_skp_image = uicontrol('style','edit',...
                                           'position',[700 20 40 20],...
                                           'string',num2str(self.nb_skipped_image),...
                                           'callback',{@(src,event)self.editNbSkippedImageCB(src,event)}); %an edit box
        ...
        function editNbSkippedImageCB(self,src,event)
             Gvalue=round(str2num(get(src,'String')));
             if (Gvalue == [])
             else
                 self.setNbSkippedImage(Gvalue);
             end
        end
        ...
        % Set Nb Skipped Image
        function setNbSkippedImage(self,v)
            if (self.video_acquisition_started==0)
                self.nb_skipped_image=v;
            end
 ...
 %              rectangle('position',self.trackers{i}.getRect()); % PB works only with one tracker!!!!
            ...
            if (self.video_acquisition_started==0)
               % if acquisition is not started, we track
               for i=1:length(self.trackers)
        ...
        function oneFrameAcquired(self,vid,event)
            ...
            if (NImage_in_Buffer > 0)
               if (self.index_skipped_image==self.nb_skipped_image)
                    self.index_skipped_image=0;
                    ...
                    if (NumImagesMovie<(self.curr_file_number+1)*self.MAX_FRAMES_PER_FILE) % If the current avi file is not too large
                        self.video_file = addframe(self.video_file, data ); % we save it in the current avi file
                    else % otherwise, the current file is too large
                        self.video_file = addframe(self.video_file, data ); % still we save current image in the current avi file
                        self.curr_file_number=self.curr_file_number+1; % Increment the file number
                        self.video_file = close(self.video_file); % closes current avi file
                   ...
                   self.index_skipped_image=self.index_skipped_image+1;
                   [data, time, metadata] = getdata(vid,1); % Gets one image from the buffer and the metadata linked to it
               end
            end
        end
        ...
        function startDevAcqu(self)
                ...
                if (self.video_acquisition_started==1)
                            ...
                            self.video_file = addframe(self.video_file, uint8(self.BG_image));
        ...
        function addTrackerDevice(self,hTD)
            ...
            set(0, 'CurrentFigure', self.hDeviceGUI);
            self.trackers{length(self.trackers)}.initDisplayedRect();
        end
